...
def closest_point(flightnum):
    ...
    ax.plot(data[:, 0] - time_difference[flightnum], np.transpose(dist), label="Mic 16", color='#540D6E', linewidth=0.8,
            zorder=1)
    ax.plot(data[:, 0] - time_difference[flightnum], np.transpose(dist_12), label="Mic 12", color='#EE4266',
    ...
    ax.axvline(x=start_time[flightnum] + expected_closest_point_time[flightnum], color='#0FA3B1', label='Theo 16',
               linewidth=1, zorder=1)
    ax.axvline(x=start_time[flightnum] + expected_closest_point_time_12[flightnum], color='#3BCEAC', label='Theo 12',
               linewidth=1, zorder=1)
    # plt.axvline(min_time - time_difference[flightnum], color ='y', label = 'PASSBY')

    # Scatter point
    if flightnum == 1 or flightnum == 2:
        ax.scatter(closestmin, dist[int((closestmin + time_difference[flightnum]) / 200)], marker="*",
                   label="GPS closest time", zorder=100, color='#6B0504', s=170)
    elif flightnum == 3 or flightnum == 5:
        ax.scatter(closestmin, dist[int((closestmin + time_difference[flightnum]) / 100)], marker="*",
                   label="GPS closest time", zorder=100, color='#6B0504', s=170)
        print(dist[int((closestmin + time_difference[flightnum]) / 100)])
    else:
        ax.scatter(closestmin, dist_12[np.argmin(dist_12)] + 9, marker="*", label="GPS closest time", color='#6B0504',
                   zorder=5000, s=170)
